...
test("verifyJwt rejects non-ObjectId sub/orgId/sid", () => {
  const t = signJwt({ ...claims, sub: "not-an-id" }, SECRET);
  expect(() => verifyJwt(t, SECRET)).toThrow(/subject/);
  const t2 = signJwt({ ...claims, orgId: "not-an-id" }, SECRET);
  expect(() => verifyJwt(t2, SECRET)).toThrow(/subject/);
  const t3 = signJwt({ ...claims, sid: "not-an-id" }, SECRET);
  expect(() => verifyJwt(t3, SECRET)).toThrow(/subject/);
});

test("verifyJwt rejects missing sid", () => {
  ...
  const data = `${header}.${payload}`;
  expect(() => verifyJwt(`${data}.${btoa("sig")}`, SECRET)).toThrow(
    /payload|signature/,
  );
});
