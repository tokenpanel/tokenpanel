...
/**
 * Factory so tests inject getDb + requireAuth without process-global hooks.
 */
export function createModelRoutes(
  ...
  modelRoutes.post(
    "/",
    requireRole("admin"),
    zValidator("json", modelCreateInput),
    async (c) => {
  ...
  modelRoutes.patch(
    "/:id",
    requireRole("admin"),
    zValidator("json", modelUpdateInput),
    async (c) => {
      const body = c.req.valid("json");
      const db = await getDb();
      const id = c.req.param("id");
      ...
      for (const [k, v] of Object.entries(body)) {
        ...
        if (k === "entries") {
          $set.entries = normalizeEntries(
            v as z.infer<typeof modelEntryInput>[],
    ...
    async (c) => {
      ...
      for (const e of body.entries) {
        if (!validIds.has(e.id)) {
          return c.json({ error: "entry_not_found", id: e.id }, 400);
        }
      }
  ...
  modelRoutes.post(
    "/:id/entries",
    requireRole("admin"),
    zValidator("json", modelEntryInput),
    async (c) => {
      const body = c.req.valid("json");
      const db = await getDb();
      const id = c.req.param("id");
