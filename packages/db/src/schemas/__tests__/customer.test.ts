...
test("re-export _balanceMoneyUnits matches moneyUnits", () => {
  expect(_balanceMoneyUnits.safeParse(100).success).toBe(true);
  expect(_balanceMoneyUnits.safeParse(-1).success).toBe(false);
});